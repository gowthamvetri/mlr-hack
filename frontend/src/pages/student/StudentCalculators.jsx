...
const StudentCalculators = () => {
    ...
    return (
        <DashboardLayout>
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-800">Grade Calculators</h1>
                <p className="text-gray-500">Calculate your SGPA and CGPA easily</p>
            </div>
            ...
            <div className="flex gap-2 mb-6">
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all ${activeTab === 'sgpa'
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all ${activeTab === 'cgpa'
                            ? 'bg-primary-600 text-white shadow-lg'
                            : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'
                        }`}
                >
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-gray-800">SGPA Calculator</h2>
                                <p className="text-sm text-gray-500">Semester Grade Point Average</p>
                            </div>
                        </div>
                        <button
                            onClick={resetSGPA}
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-6">
                        <p className="text-sm text-blue-700">
                            <strong>Formula:</strong> SGPA = Σ(Credits × Grade Point) ÷ Σ Credits
                        </p>
                    </div>
                    ...
                    <div className="space-y-3 mb-6">
                        ...
                        {sgpaSubjects.map((subject, index) => (
                            <div key={subject.id} className="grid grid-cols-12 gap-3 items-center bg-gray-50 p-3 rounded-lg">
                    ...
                    <div className="flex gap-3 mb-6">
                        <button
                            onClick={addSubject}
                            className="flex items-center gap-2 px-4 py-2 border border-dashed border-gray-300 text-gray-600 hover:bg-gray-50 rounded-lg transition-colors"
                        >
                    ...
                    {/* SGPA Result */}
                    {sgpaResult && (
                        <div className={`p-6 rounded-xl ${sgpaResult.error ? 'bg-red-50 border border-red-200' : 'bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200'}`}>
                            {sgpaResult.error ? (
                                <p className="text-red-600 font-medium">{sgpaResult.error}</p>
                            ) : (
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="text-sm text-gray-600 mb-1">Your SGPA</p>
                                        <p className="text-4xl font-bold text-green-600">{sgpaResult.sgpa}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-sm text-gray-500">Total Credits: <span className="font-semibold text-gray-700">{sgpaResult.totalCredits}</span></p>
                                        <p className="text-sm text-gray-500">Total Points: <span className="font-semibold text-gray-700">{sgpaResult.totalCreditPoints}</span></p>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-bold text-gray-800">CGPA Calculator</h2>
                                <p className="text-sm text-gray-500">Cumulative Grade Point Average</p>
                            </div>
                        </div>
                        <button
                            onClick={resetCGPA}
                    ...
                    <div className="space-y-3 mb-6">
                        ...
                        {semesters.map((sem) => (
                            <div key={sem.id} className="grid grid-cols-12 gap-3 items-center bg-gray-50 p-3 rounded-lg">
                    ...
                    {/* CGPA Result */}
                    {cgpaResult && (
                        <div className={`p-6 rounded-xl ${cgpaResult.error ? 'bg-red-50 border border-red-200' : 'bg-gradient-to-r from-purple-50 to-violet-50 border border-purple-200'}`}>
                            {cgpaResult.error ? (
                                <p className="text-red-600 font-medium">{cgpaResult.error}</p>
                            ) : (
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="text-sm text-gray-600 mb-1">Your CGPA</p>
                                        <p className="text-4xl font-bold text-purple-600">{cgpaResult.cgpa}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-sm text-gray-500">Semesters: <span className="font-semibold text-gray-700">{cgpaResult.semesterCount}</span></p>
                                        <p className="text-sm text-gray-500">Total Credits: <span className="font-semibold text-gray-700">{cgpaResult.totalCredits}</span></p>
                                    </div>
                                </div>
                            )}
                        </div>
