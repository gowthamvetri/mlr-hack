...
const CountBadge = ({ count, active = false }) => (
    ...
    ${active
            ...
            : 'bg-zinc-100 text-zinc-500 group-hover:bg-violet-50 group-hover:text-violet-600'
        }
  `}>
        {count}
    </span>
...
const FilterChip = ({ label, icon: Icon, active, onClick, count, color = 'violet' }) => {
    const colorStyles = {
        violet: { active: 'bg-violet-600 text-white shadow-lg shadow-violet-500/20', hover: 'hover:border-violet-300 hover:bg-violet-50' },
        blue: { active: 'bg-blue-600 text-white shadow-lg shadow-blue-500/20', hover: 'hover:border-blue-300 hover:bg-blue-50' },
        emerald: { active: 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/20', hover: 'hover:border-emerald-300 hover:bg-emerald-50' },
        amber: { active: 'bg-amber-600 text-white shadow-lg shadow-amber-500/20', hover: 'hover:border-amber-300 hover:bg-amber-50' },
        zinc: { active: 'bg-zinc-800 text-white shadow-lg shadow-zinc-500/20', hover: 'hover:border-zinc-300 hover:bg-zinc-100' },
        red: { active: 'bg-red-600 text-white shadow-lg shadow-red-500/20', hover: 'hover:border-red-300 hover:bg-red-50' },
    };
    ...
    return (
        ...
        ${active
                    ...
                    : `bg-white border-zinc-200 text-zinc-600 ${styles.hover}`
                }
      `}
        >
        ...
        <button
            ...
            className={`
                ...
                ${isOpen
                    ...
                    : 'bg-white border-zinc-200 text-zinc-600 hover:border-violet-300 hover:bg-violet-50 hover:text-violet-700 shadow-sm'
                }
                ${className}
            `}
        >
            ...
            {activeFiltersCount > 0 && (
                <span className={`
                    ...
                    ${isOpen ? 'bg-white text-violet-600' : 'bg-violet-100 text-violet-700'}
                `}>
                    {activeFiltersCount}
                </span>
            )}
...
export const PremiumFilterBar = ({
    ...
    return (
        <div
            ...
            className={`
                relative bg-white rounded-2xl border border-zinc-200
        ...
        >
            ...
            <div className="px-4 lg:px-5 py-3 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
                <div className="flex items-center gap-3">
                    <div className="p-2 bg-violet-100/50 rounded-lg">
            ...
            <div ref={contentRef} className="p-4 lg:p-5">
                    ...
                    <div className="mb-4">
                        <div className="relative group">
                            ...
                            <input
                                ...
                                className="
                                    w-full pl-10 pr-10 py-3 text-sm bg-zinc-50 border border-zinc-200 rounded-xl
                                    focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500
                                    transition-all duration-200 text-zinc-900 placeholder-zinc-400
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    ...
                    {departments.length > 0 && (
                        <div className="space-y-2.5 p-3 bg-zinc-50/50 rounded-xl border border-zinc-100">
            ...
            <div className="px-4 lg:px-5 py-3 bg-zinc-50 border-t border-zinc-100 flex items-center justify-between rounded-b-2xl">
                <div className="flex items-center gap-3">
                    {filteredCount !== undefined && totalCount !== undefined && (
                        <p className="text-xs text-zinc-500">
                            Showing <span className="font-bold text-zinc-900">{filteredCount}</span> of{' '}
                            <span className="font-bold text-zinc-900">{totalCount}</span> results
                        </p>
                    )}
                    {hasActiveFilters && (
                        <span className="flex items-center gap-1 text-[10px] font-bold text-violet-600 bg-violet-100 px-2 py-0.5 rounded-full">
                ...
                <div className="flex items-center gap-2">
                    {hasActiveFilters && onClearFilters && (
                        <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 rounded-lg transition-colors border border-red-100"
                        >
                            <X className="w-3 h-3" />
                            Clear all
                        </button>
                    ...
                    <button
                        onClick={onClose}
                        className="flex items-center gap-1.5 px-4 py-1.5 text-xs font-bold text-white bg-violet-600 hover:bg-violet-700 rounded-lg transition-colors shadow-sm shadow-violet-200"
                    >
                        Apply Filters
                    </button>
                </div>
