...
Ask me anything about MLRIT!"""
    ...
    def _compile_patterns(self):
        for attr in dir(self):
            # Skip private attributes (like _compile_patterns itself) and only get pattern lists
            if attr.endswith("_patterns") and not attr.startswith("_") and not attr.startswith("compiled"):
                patterns = getattr(self, attr)
                if isinstance(patterns, list):  # Extra safety check
